...
// Handler 处理客户端连接
func (r *RESPHandler) Handler(ctx context.Context, client resp.Connection) {
	if r.closing.Load() { // 如果当前处于关闭状态，关闭连接
		_ = client.Close()
		return
	}
	data, err := client.GetConn().Next(-1)
	if err != nil {
		logger.Error("read from client error: " + err.Error())
	...
	for last < len(data) {
		payload, _, _, last = parser.ParseStream(data[last:], last) // 解析客户端请求
		...
		if payload.Err != nil { // 解析出错
			...
			if err != nil {
				r.closeClient(client)
				logger.Info("Connection closed: " + client.GetConn().RemoteAddr().String())
				return
			}
		...
		if payload.Data == nil { // 指令为空
			logger.Info("request is nil: " + client.GetConn().RemoteAddr().String())
			return
		}
		//客户端发送的指令必须是二维数组格式的
...
func (r *RESPHandler) closeClient(client resp.Connection) {
